...
class TestProfileForm:
    ...
    def test_clean_email_allows_own_email(self):
        ...
        form = ProfileForm(
            {"username": "myuser", "email": "test@example.com"}, instance=user
        )
        assert form.is_valid()
    ...
    def test_clean_email_rejects_taken_email(self):
        ...
        form = ProfileForm(
            {"username": "myuser", "email": "taken@example.com"}, instance=user
        )
        assert not form.is_valid()
    ...
    def test_clean_email_allows_empty(self):
        user = UserFactory.create(email="test@example.com")
        form = ProfileForm({"username": "myuser", "email": ""}, instance=user)
        assert form.is_valid()
